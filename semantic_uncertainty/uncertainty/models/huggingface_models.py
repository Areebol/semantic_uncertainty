...
class HuggingfaceModel(BaseModel):
    ...
    def beam_predict(self, input_data, temperature, num_sample, do_sample = False, return_full=False):
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                num_beams=num_sample, # Enable beam search / sample
                num_return_sequences=num_sample, # Enable beam search / sample
                max_new_tokens=self.max_new_tokens,
                return_dict_in_generate=True,
                output_scores=True,
                output_hidden_states=True,
        ...
        for index in range(num_sample):
            if len(outputs.sequences[index]) > self.token_limit:
                raise ValueError(
                    'Generation exceeding token limit %d > %d',
                    len(outputs.sequences[index]), self.token_limit)

            full_answer = self.tokenizer.decode(
                outputs.sequences[index], skip_special_tokens=True)
            ...
            # Remove whitespaces from answer (in particular from beginning.)
            sliced_answer = sliced_answer.strip()
            sliced_answers.append(sliced_answer)
